...
export default function () {
  ...
  let pedidoRes = http.post(
    'http://localhost:5180/api/pedidos/asincrono',
    JSON.stringify({ carritoId: carrito.id }),
    { headers: { 'Content-Type': 'application/json' } }
  );

  let ok = check(pedidoRes, {
    'status 200 o 202': (r) => r.status === 200 || r.status === 202,

    'respuesta no vacía': (r) => r.body && r.body.length > 0,

    'json válido (opcional)': (r) => {
      try {
        r.json();
        return true;
      } catch (e) {
  ...
  if (!ok) {
    console.error('Error en pedido async:', pedidoRes.status, pedidoRes.body);
  }
